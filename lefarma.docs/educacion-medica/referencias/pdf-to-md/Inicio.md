---
codigo: "—"
version: "—"
fecha: "—"
area: "General"
tipo: "Índice"
---

# Wiki de Educación Médica y Ventas IMSS — Lefarma

Bienvenido a la documentación de los procesos de [[Educación Médica]] y [[Ventas IMSS]]. Esta wiki reúne diagramas, procedimientos, instructivos, formularios y reportes convertidos a Markdown desde los PDFs oficiales de Asokam / Lefarma (originales en `referencias/pdf/`).

## Áreas

- [[Educación Médica]] — Talleres médicos en hospitales, material, selección de hospitales, logística e impartición.
- [[Ventas IMSS]] — Proceso de ventas al IMSS, plan de trabajo, visitas a unidades médicas e indicadores.

## Mapa por tipo de documento

### Procesos

- [[Talleres Médicos en Hospitales]]
- [[Proceso de Ventas IMSS]]

### Procedimientos

- [[Procedimiento Normalizado de Operación del Proceso de Ventas]]

### Instructivos

- [[Preparación y Autorización de Material de Talleres Médicos]]
- [[Elaboración del Concentrado Anual de Talleres Médicos]]
- [[Selección Mensual de Hospitales para Talleres Médicos]]
- [[Solicitud y Entrega de Materiales para Talleres Médicos]]
- [[Impartición de Talleres Médicos]]
- [[Elaborar Plan de Trabajo]]
- [[Visitar Unidades Médicas del IMSS]]
- [[Recabar Información e Indicadores de Ventas IMSS]]

### Formularios

- [[Formularios|Ver catálogo completo de formularios]]
- [[ASK-CEM-FOR-002 Base de Datos de Hospitales]]
- [[ASK-CEM-FOR-003 Programa Anual de Talleres Médicos]]
- [[ASK-CEM-FOR-004 Selección de Hospitales para Talleres Médicos]]
- [[ASK-CEM-FOR-005 Matriz de Talleres Médicos]]
- [[ASK-CEM-FOR-006 Calendario de Talleres Médicos]]
- [[ASK-CEM-FOR-007 Material para Talleres Médicos]]
- [[ASK-CEM-FOR-008 Registro de Asistencia]]
- [[ASK-VEN-FOR-001 Metas de Ventas y Desplazamiento]]
- [[ASK-VEN-FOR-003 Reporte de Visitas Médicas]]

### Diagramas

- [[Diagrama del Proceso de Talleres Médicos]]
- [[Diagrama del Proceso de Ventas]]

### Reportes

- [[Reporte de Visitas Médicas]]

### Roles

- [[Roles y Abreviaturas]]

### Referencias (documentos citados no incluidos)

Documentos citados en los procesos que **no forman parte** del set digitalizado. Son nodos de trazabilidad para mantener todos los enlaces resueltos:

- [[ASK-ADM-FOR-001 Solicitud de Viáticos]]
- [[ASK-CBA-FOR-001 Plantilla de Presentación Asokam]]
- [[ASK-CEM-ANE-001 Speech para promover el Taller Médico]]
- [[ASK-CEM-FOR-001 Base de datos de Hospitales del Sistema Público de Salud]]
- [[ASK-GGE-IDT-001 Solicitud y aprobación de viáticos]]
- [[ASK-TES-FOR-001 Depósito Box Lunch para Talleres Médicos IMSS y Descentralizados]]
- [[ASK-VEN-DOE-001 Reporte de Incidente]]
- [[ASK-VEN-FOR-002 Plan de Trabajo (formato)]]
- [[ASK-VEN-FOR-004 Reporte Semanal de Visitas Hospitalarias sección IMSS]]
- [[ASK-VEN-FOR-005 Indicador Cumplimiento del Plan de Trabajo]]
- [[ASK-VEN-ANX-001 Base de datos para Reporte de Desplazamiento]]
- [[ASK-VEN-ANX-002 Base de datos para Indicadores de Ventas]]
- [[ASK-COM-FOR-003 Indicador Cumplimiento del Plan de Trabajo]]
- [[ASK-KPI-VEN-001 Cumplimiento Plan de Trabajo IMSS (Indicador)]]

## Cómo usar esta wiki

- Navega por las notas usando los enlaces `[[...]]`.
- Consulta [[Roles y Abreviaturas]] para entender las siglas de los actores.
- Revisa el [[Formularios|catálogo de formularios]] para localizar códigos ASK-CEM-FOR y ASK-VEN-FOR.
- Los diagramas de flujo se conservan en sintaxis Mermaid y pueden visualizarse en Obsidian o cualquier visor compatible.
