...
Read `package.json` with the ReadFile tool. Extract:
- `typescript` version
...
Use FetchURL for each. Skim for hook signatures, type utility behavior, and any deprecation notices.
...
```
SearchWeb: "[feature name] [library] [version] typescript official docs 2025"
```
...
Use `ReadFile` on each file completely. Use `Glob` to discover related files if paths are not explicit.
