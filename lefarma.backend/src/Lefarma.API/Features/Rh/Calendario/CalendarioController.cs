...
public class CalendarioController : ControllerBase
{
    ...
    [HttpGet("mis-dias-jornada")]
    [SwaggerOperation(Summary = "Obtener días de jornada laboral del usuario autenticado")]
    public async Task<IActionResult> ObtenerDiasJornada([FromQuery] DiasJornadaRequest request)
    {
        var idUsuario = int.TryParse(User.FindFirstValue(ClaimTypes.NameIdentifier), out var id) ? id : 0;
        var result = await _service.ObtenerDiasJornadaAsync(request.Anio, request.Mes, idUsuario);
        return result.ToActionResult(this, data => Ok(new ApiResponse<DiasJornadaResponse>
        { Success = true, Message = "Días de jornada laboral obtenidos exitosamente.", Data = data }));
    }
}
