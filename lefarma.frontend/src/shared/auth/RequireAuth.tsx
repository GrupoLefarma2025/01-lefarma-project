...
/**
 * Route guard for the `/CxP/` base-app shell and any mounted app subtree.
 *
 ...
 * is sufficient (no full-page reload). The root build does NOT use this guard —
 * it has its own `ProtectedRoute` — so this change does not affect root-build
 * behavior.
 *
 * The `?return=` form integrates with `Login.tsx`'s existing `safeReturn`
 * handling (reads `window.location.search`, scoped to `/CxP/` as an
 * open-redirect guard). No changes to `Login.tsx` are required.
 *
...
export interface RequireAuthProps {
   ...
   * subtrees override with their own login (e.g. `/gastos/login`).
   */
  loginPath?: string;
}
...
export function RequireAuth({ children, loginPath = '/login' }: RequireAuthProps) {
  ...
  if (!isAuthenticated) {
    // In-app redirect within the single-bundle shell. The root build is unaffected
    // (it uses its own `ProtectedRoute` and never mounts `RequireAuth`).
    //
